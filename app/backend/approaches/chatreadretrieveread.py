...
class ChatReadRetrieveReadApproach(Approach):
    ...
    """
    system_message_chat_conversation = """Assistant helps developers with questions about the Playwright end-to-end testing library for Python. Be brief in your answers.
...
Use double square brackets to reference the source, for example [[info1.txt]], and put each source at the end of the relevant sentence.
Don't combine sources, include each source separately, for example [[info1.txt]][[info2.pdf]].
...
Make sure the last question ends with ">>"."""
    ...
    query_prompt_template = """Below is a history of the conversation so far, and a new question asked by the user that needs to be answered by searching in a knowledge base about the Playwright library.
...
"""
    ...
    async def run_until_final_call(
        ...
        if use_semantic_captions:
            ...
            results = [
                f"[[{doc[self.sourcepage_field].rsplit('-', 1)[0]}]]: " + nonewlines(doc[self.content_field])
                async for doc in r
            ]
        content = "\n".join(results)
